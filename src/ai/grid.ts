...
/** Простая точка в мировых координатах. Структурно совместима с Phaser.Math.Vector2. */
...
/**
 * Phaser-free ядро навигации: сетка проходимости + Theta* (any-angle) поиск
 * пути и DDA line-of-sight. Работает только на числах/Cell/Vec2 — тестируется
 * без движка. Обёртку с Phaser.Math.Vector2 даёт Pathfinder.
 */
export class Grid {
  ...
  constructor(grid: boolean[][], cols: number, rows: number, cellSize: number) {
    this.grid = grid;
    this.cols = cols;
    this.rows = rows;
  ...
  /** Строит сетку проходимости из прямоугольников стен. */
  static fromWalls(walls: WallDef[], mapW: number, mapH: number, cellSize = PATH_CELL_SIZE): Grid {
    const cols = Math.ceil(mapW / cellSize);
    const rows = Math.ceil(mapH / cellSize);

    // Initialize all cells as walkable
    const grid = Array.from({ length: rows }, () => new Array<boolean>(cols).fill(true));
    ...
    for (const wall of walls) {
      ...
      const colMax = Math.min(cols - 1, Math.floor((right - 1) / cellSize));
      const rowMin = Math.max(0, Math.floor(top / cellSize));
      const rowMax = Math.min(rows - 1, Math.floor((bottom - 1) / cellSize));
    ...
    return new Grid(grid, cols, rows, cellSize);
  }
  ...
  findPath(fromX: number, fromY: number, toX: number, toY: number): Vec2[] {
    ...
    start.col = clamp(start.col, 0, this.cols - 1);
    start.row = clamp(start.row, 0, this.rows - 1);
    end.col = clamp(end.col, 0, this.cols - 1);
    end.row = clamp(end.row, 0, this.rows - 1);
    ...
    const waypoints: Vec2[] = cells.map((c) => this.cellToWorld(c));
   ...
   */
  private trimStart(fromX: number, fromY: number, waypoints: Vec2[]): Vec2[] {
    let first = 0;
   ...
   */
  gridLoS(ax: number, ay: number, bx: number, by: number): boolean {
  ...
  nearestWalkable(center: Cell): Cell | null {
   ...
   * entire grid is blocked. RNG injectable for deterministic tests.
   */
  randomWalkableWorld(rng: () => number = Math.random): Vec2 | null {
