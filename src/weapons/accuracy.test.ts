...
describe("jitterAngle", () => {
  ...
  it("неточность стрелка и неточность ствола мерятся одной единицей", () => {
    // Инвариант ценен именно так: обе константы прогоняются через один и тот же код,
    // поэтому «полный конус» не может разойтись между стволом и стрелком. Величину
    // самой константы держит config.test.ts — здесь проверяется только единица.
