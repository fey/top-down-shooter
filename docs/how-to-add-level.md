...
| `smart`         | любое     | `SmartBot` — соперник уровня игрока |
| `weapon_pickup` | любое     | Пикап оружия; какое именно — в строковом свойстве `weapon`: `pistol`, `shotgun`, `rifle`, `automat` |

Если `player_start` отсутствует, игра выдаст предупреждение и поставит игрока в (100, 100).

Идентификатор не из таблицы (опечатка в Class, старое название вроде `ranged`) — тоже
предупреждение в консоли с координатами объекта; сущности из него не появится.
