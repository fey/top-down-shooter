...
Неизвестный **идентификатор спавна** (не пикап) пока игнорируется без диагностики: `createEnemy`
возвращает `null`, в консоли ничего. Такой объект на карте есть (`ranged` на `level2`) — врага из
него не появляется. Это известное поведение, а не свойство формата.
