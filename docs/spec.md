...
(`glyph`, `cooldown`, `bulletSpeed`, `pelletCount`, `spreadRad`, `barrel`), а единственный класс
`Weapon` его исполняет. Добавить пушку = добавить запись в реестр: текстура игрока и текстура
пикапа генерируются из `barrel` и `glyph` автоматически.
...
- **Пистолет**: 1 пуля, кулдаун 250 мс, скорость 600 px/s.
- **Дробовик**: 5 дробинок в веере ~30° (±15°), кулдаун 700 мс — плата за плотность огня.
- `SmartBot` использует тот же `Weapon` с `WEAPONS.pistol`; враги-стрелки стреляют своим кодом
  (`ShooterEnemy.spawnBullet`) и реестра не используют.
...
и **глифом** оружия по центру (`WeaponDef.glyph`: пистолет — `P`, дробовик — `S`). Цвет отвечает
