...
Браузерный прототип top-down шутера: статичные уровни из Tiled, три типа врагов, два оружия (пистолет и дробовик). Этот документ описывает **то, что реализовано сейчас**. Оставшаяся работа — в [`docs/roadmap.md`](roadmap.md).
...
отталкивание от стен, кулдаун, веер дробинок, парсинг уровня) вынесена в чистые Phaser-free
модули (`src/ai/grid.ts`, `src/ai/geometry.ts`, `src/ai/separation.ts`, `src/ai/behaviors/`,
`src/weapons/cooldown.ts`, `src/weapons/pellets.ts`, `src/level/spawns.ts`) и покрыта
unit-тестами vitest. Классы-сущности — тонкая оболочка
...
top-down-shooter/
  ...
  src/
    ...
    weapons/
      ...
      cooldown.ts          # чистое ядро: проверка кулдауна
    ai/
...
- **Веер** считает чистая `computePelletAngles(baseAngle, pelletCount, spreadRad)`
  (`weapons/pellets.ts`): `spreadRad` — полный угол от крайней левой до крайней правой дробинки,
  при `pelletCount ≤ 1` — одиночный выстрел точно по прицелу.
- **Неточность** — другое понятие: `jitterAngle(baseAngle, aimSpreadRad, roll)`
  (`weapons/accuracy.ts`) случайно уводит выстрел внутри конуса `aimSpreadRad`. Розыгрыш —
  один на выстрел и применяется **до** раскладки веера, поэтому дробь одного патрона летит
  связным конусом. `Math.random` живёт в `Weapon`, ядро принимает `roll` параметром.
...
У объекта `weapon_pickup` оружие задаётся пользовательским свойством `weapon` (`pistol` / `shotgun`).
Чистые функции разбора — в `level/spawns.ts`: `classifySpawn` (идентификатор → тип спавна) и
`readPickupWeaponId` (свойства объекта → id оружия). Неизвестное или отсутствующее оружие пикапа
логируется предупреждением и пропускается — битые данные карты не валят загрузку уровня.
