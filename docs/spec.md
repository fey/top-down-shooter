...
(`cooldown`, `bulletSpeed`, `pelletCount`, `spreadRad`, `barrel`), а единственный класс `Weapon`
его исполняет. Добавить пушку = добавить запись в реестр: текстура игрока и цвет пикапа
генерируются из `barrel` автоматически.
...
`staticGroup`: квадрат со светлой рамкой и заливкой цветом ствола того оружия, что в нём лежит.
